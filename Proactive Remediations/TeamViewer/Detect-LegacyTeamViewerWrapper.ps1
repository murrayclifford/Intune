<#
    .SYNOPSIS
    Searches registry for TeamViewer MSI Wrapper installations
#>
...
try {
    foreach ($Path in $RegUninstallPaths){
        ...
        ForEach-Object {
            Write-Output "Non Compliant: TeamViewer MSI Wrapper installation found on device"
            Stop-Transcript
            Exit 1
        }
    }
    Write-Output "Compliant: eamViewer MSI Wrapper installation not found on device"
    Stop-Transcript
    Exit 0
}
catch {
